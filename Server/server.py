...
class Registration:
    ...
    def __verificate(self, username):
        try:
            # We are waiting for the code word from the user
            self.protocol.send("verification", self.connection.socket)
            data = self.protocol.recv(self.connection.socket).decode('utf-8')
        except Exception:
    ...
    def __signup(self, data):
        ...
        if self.setting.enable_password:
            ...
            if self.__verificate(username, self.connection):
                return True
            else:
                return False
        else:
    ...
    def _signin(self, data=None):
        '''This method is responsible for authorizing the client if it was registered,\n 
        and for registering if the client is not found.'''
        if not data:
            if self.protocol.recv(self.connection.socket).decode('utf-8') == "wanna_connect":
                self.protocol.send("server_puplic_key", self.connection.socket)
                self.protocol.send(self.setting.public_key, self.connection.socket)
                self.protocol.send("userdata", self.connection.socket)
                data = self.protocol.recv(self.connection.socket)
        ...
        if self.server_database.is_user_already_exist(username):
            if self.server_database.is_user_verificated(username):
                if self.server_database.is_keys_match(username, public_key):
                    self.protocol.send("success", self.connection.socket)
                    key = self.setting.encrypt_key(self.aes_key, public_key)
                    self.protocol.send(key, self.connection.socket)
                    return True
                else:
                    self.protocol.send('key_error', self.connection.socket)
                    return False
            else:
                if self.__verificate(username):
                    self._signin([username, str(public_key)])
                    return True
                else:
                    self.protocol.send('verification_error', self.connection.socket)
                    return False
        else:
            if self.__signup([username, str(public_key)]):
                self._signin([username, str(public_key)])
                return True
            else:
                # Unknown error.
...
class Server(socket.socket):
    ...
    def __handler(self, client_connection:Connection):
        while self.state == STATE_WORKING and getattr(client_connection.thread, "do_run", True):
            try:
                data = self.protocol.recv(client_connection.socket)  # Reading client
                if self.isCrypted(data):
                    data = self.setting.AES.decrypt(data).decode('utf-8') # If message is crypted, decrypting
                else:
    ...
    def __connect(self, client_data:Connection):
        ...
        if Registration(client_data, self.setting)._signin():
