...
class RSACrypt:
    ...
    def encrypt(self, msg, publickey=None):
        encryptor = PKCS1_OAEP.new(self.__private_key.publickey()) if not publickey else PKCS1_OAEP.new(publickey)
        return b64encode(encryptor.encrypt(msg.encode('utf-8')))
    
    def decrypt(self, encrypted, privatekey=None):
        decryptor = PKCS1_OAEP.new(self.__private_key) if not privatekey else PKCS1_OAEP.new(privatekey)
